...
const LargeDock = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden h-20 items-start gap-4 rounded-2xl bg-white/25 px-4 pt-4 dark:bg-black/25 md:flex", // Large dock styles
        className,
...
function DockIcon({
  ...
  const widthTransform = useTransform(distanceFromMouse, [-150, 0, 150], [50, 80, 50]);
  const heightTransform = useTransform(distanceFromMouse, [-150, 0, 150], [50, 80, 50]);

  // Transform properties for icon size based on mouse distance
  const iconWidthTransform = useTransform(distanceFromMouse, [-150, 0, 150], [30, 50, 30]);
  const iconHeightTransform = useTransform(distanceFromMouse, [-150, 0, 150], [30, 50, 30]);
        ...
        <AnimatePresence>
          ...
          {isHovered && (
            <motion.div
              ...
              className="absolute -bottom-5 w-fit translate-x-1/2 rounded-md border border-gray-200 bg-white/80 px-2 py-0.5 text-xs text-neutral-700 dark:border-neutral-900 dark:bg-neutral-800 dark:text-white"
            >
              {title} {/* Tooltip text */}
            </motion.div>
          )}
    ...
    <div className={cn("relative block md:hidden", className)}>
      <AnimatePresence>
        ...
        {isOpen && (
          <motion.div
            layoutId="nav"
            className="absolute inset-y-0 bottom-full mb-2 flex flex-col gap-2"
          >
            {items.map((item, index) => (
              <motion.div
                ...
                transition={{ delay: (index) * 0.05 }} // Delay for exit animations
              >
                <Link
                  href={item.href}
                  key={item.title}
