...
export default function ProjectSwapTextCard({ initialText, finalText, projectObj }: FlipTextCardProps) {
  return (
    <div className={`relative group flex min-h-64 w-full flex-col justify-between rounded-3xl bg-white/10 dark:bg-black/10 border-2 border-white dark:border-black p-6 mt-5 md:max-w-[500px] hover:scale-105 transition duration-100 ${projectObj.base_className} overflow-hidden`}>
      ...
      <div className="flex flex-col justify-between md:min-w-72">
        <div className="md:hidden">
          ...
          <div className="text-sm font-medium text-gray-500 dark:text-gray-100">{finalText}</div>
        </div>
        <SwapText
          ...
          finalTextClassName="text-sm h-full duration-200 font-medium text-gray-500 dark:text-gray-100"
        />
      </div>
    </div>
  );
